...
/* Winsock: socket handle from sockets_win.c */
#ifdef _WIN32
 ...
 *
 * WIN32 PORT: Uses recv() on g_wais_socket instead of read() on fd.
 *             The 'd' parameter is ignored on Windows.
 */
long read_from_stream(d,buf,nbytes)
long d;				/* this is the stream (ignored on Win32) */
char *buf;
long nbytes;
{
  long didRead;
  ...
  while (toRead > 0){
    ...
    didRead = recv(g_wais_socket, buf, (int)toRead, 0);
    if(didRead == SOCKET_ERROR)
      return(-1);
#else
...
long response_buffer_length;
{
...
#ifdef _WIN32
  /* WIN32 PORT: Use Winsock send() instead of fwrite on FILE* */
  {
    long total = request_length + HEADER_LENGTH;
    long sent = 0;
    while (sent < total) {
      int rc = send(g_wais_socket, request_message + sent,
                    (int)(total - sent), 0);
  ...
  while(1){
    if(0 > read_from_stream(0, response_message, 1))
      return 0;
    if('0' == response_message[0])
      break;
  }

  if(0 > read_from_stream(0, 
				  response_message + 1, 
				  HEADER_LENGTH - 1))
    return 0;
...
#ifdef _WIN32
  /* WIN32 PORT: Socket close handled by close_connection_to_server() */
  /* Do NOT fclose the sentinel FILE* (stderr) */
