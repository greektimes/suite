 ...
 *   This file is a verbatim port of FFmpeg's libavcodec/truespeech_data.h.
 *   It carries only the codec's static lookup tables (functional constants
 *   reverse-engineered by Konstantin Shishkov) and is used by the ported
 *   src/truespeech_decoder.c. The tables are unchanged from upstream; only
 *   the include guard name has been adjusted to avoid collision. The LGPL
 *   header above is retained as required by the license.
 * ---------------------------------------------------------------------------
 */

#ifndef MGT_TRUESPEECH_DATA_H
#define MGT_TRUESPEECH_DATA_H
