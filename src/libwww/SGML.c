		...
		switch(context->state)
			...
			*/
			...
			if (isalnum((int)c))
			    ...
			    {
				/* Numeric character reference. Parse hex when
				 * the chunk starts with x/X (&#x...;), else
				 * decimal. On success emit the codepoint as
				 * its 1..4-byte UTF-8 sequence one byte at a
				 * time so codepoints above U+007F survive the
				 * single-byte sink. */
				unsigned int value = 0;
				int parsed_ok = 0;
				const char *digits;
				HTChunk_terminate(string);
				digits = HTChunk_data(string);
				if (digits && (digits[0] == 'x' || digits[0] == 'X')) {
				    if (sscanf(digits + 1, "%x", &value) == 1)
					parsed_ok = 1;
				} else if (digits) {
				    if (sscanf(digits, "%u", &value) == 1)
					parsed_ok = 1;
				}
				if (parsed_ok && value < 0x110000U) {
				    /* Emit the full UTF-8 sequence in one
				     * PUTB call so the downstream stream
				     * sees the multi-byte run as a unit. A
				     * per-byte PUTC would split a 2-/3-/4-
				     * byte codepoint across separate
				     * put_character invocations, defeating
				     * any UTF-8 validity sniff on the
				     * other side. */
				    char utf8[4];
				    int  utf8_n;
				    if (value < 0x80U) {
					...
					utf8[0] = (char)(0xC0U | (value >> 6));
					utf8[1] = (char)(0x80U | (value & 0x3FU));
					utf8_n = 2;
				    } else if (value < 0x10000U) {
					utf8[0] = (char)(0xE0U | (value >> 12));
					utf8[1] = (char)(0x80U | ((value >> 6) & 0x3FU));
					utf8[2] = (char)(0x80U | (value & 0x3FU));
					utf8_n = 3;
				    } else {
					utf8[0] = (char)(0xF0U | (value >> 18));
					utf8[1] = (char)(0x80U | ((value >> 12) & 0x3FU));
					utf8[2] = (char)(0x80U | ((value >> 6) & 0x3FU));
					utf8[3] = (char)(0x80U | (value & 0x3FU));
					utf8_n = 4;
				    }
				    PUTB(utf8, utf8_n);
				} else {
